    ...
    $.get('/filesList', {folder: '/storage'}, function(data) {
      ...
      var filesListClickHandler = function() {
        ...
        if($item.data('type') == 'file') {
          ...
          $.get('/filesList', {folder: '/storage/' + $item.find('.title').text()}, function(data) {
      ...
      $modalElem.on('click', '.files_list_wrp .navigation .back', function() {
        if(currPath.length > 1) {
          currPath.splice(-1, 1);
          $.get('/filesList', {folder: currPath.join('/')}, function(data) {
            $modalElem.find('.files_list_wrp').replaceWith(filesListTemplate(data));
          });
        }
      });
      ...
      $modalElem.on('change', 'input[type="file"]', function() {
        ...
        sendFiles(files, {folder: '/storage'}, function(err/*, data*/) {
          if(err) {
            return alert('err!');
          }
